...
{
    public class IdGenerationUtlity
    {
        int PPBCode = 7;
        int MOCCode = 8;
        int OBXCode = 9;
        ...
        private List<string> generateCryptoCode(int Quantity, int LengthOfUid)
        {
            try
            {
                IDGenrationFactory obj = new IDGenrationFactory();
                return obj.generateCryptoCode(Quantity, LengthOfUid);
            }
            catch (Exception)
            {
        ...
        public void generateIds(int RequestId, int JobId, int PAID, string selectedJobType, string IAC_CIN)
        {
            try
            {
                ...
                if (UIDQuantity != 0)
                {
                    CalculateAndGenerateAndSaveIds(RequestId, selectedJobType);
                }
                ///Generate and Save SSCC
                CalculateSSCCToPrint();
                GenerateAndSaveSSCC(JobId, RequestId, selectedJobType, IAC_CIN);
            }
            catch (Exception)
            {
