    ...
    this.update = function (activeElement) {
        ...
        if (this.activeElement == activeElement) {
            ...
            for (var key in activeElement) {
                if (activeElement[key] == 'label_text') {
                    if (isNaN(activeElement.fontSize)) {
                        activeElement.fontSize = 14;
                    }
                }

                if (isNaN(activeElement.width)) {
                    if (key != 'matrixType') {
                        activeElement.width = 0;
                    } else {
                        activeElement.width = 14;
                    }
                }
                if (isNaN(activeElement.height)) {
                    if (key != 'matrixType') {
                        activeElement.height = 0;
                    } else {
                        activeElement.height = 14;
                    }

                }

                if (isNaN(activeElement.x)) {
                    activeElement.x = 0;
                }
                if (isNaN(activeElement.y)) {
                    activeElement.y = 0;
                }
                //if (isNaN(activeElement.lineSpacing)) {
                //    activeElement.lineSpacing = 14;
                //}
                ...
                if (!activeElement.readonly || key != "readonly" && $.inArray(key, activeElement.readonly) == -1) {
                    if (getType.toString.call(activeElement[key]) != '[object Function]') {
                        ...
                        if (key == 'type') {
                            ...
                            if (key == 'lineSpacing') {
                                if (activeElement.hidden[0] != 'matrixType') {
                        ...
                        if (key != 'type' && key != 'mil') {
                            ...
                            if (activeElement[key] == "Barcode") {
                                ...
                                var options = [{ value: 'LabelText', title: 'Label Text' }
                                    ...
                                    , { value: 'MACHINENO', title: 'Machine Number' }
