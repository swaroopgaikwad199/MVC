...
com.logicpartners.labelDesigner = function (canvasid, labelWidth, labelHeight) {
    ...
    this.resize = function (xchange, ychange) {
        ...
        if (this.activeElement.height < 0) {
            ...
            this.activeElement.setHeight(this.activeElement.getHeight());
    ...
    this.updateCanvas = function () {
        ...
        var canvasG = document.getElementById("labelDesigner"),
            context = canvasG.getContext("2d"),
            width = canvasG.width,
            height = canvasG.height;
        grid.square(context, this.labelX, this.labelY, width, height, 50, 25, 12.5);
    }
    ...
    this.generateZPL = function () {
        ...
        var lblWidth = Math.round((this.labelWidth / 100) * parseInt(selDPI));
        var lblheight = Math.round((this.labelHeight / 100) * parseInt(selDPI));
    ...
    this.getZPL = function () {
        ...
        var data = "^XA\r\n" +
            ...
            //"^MD30\r\n" +
            "^CI28\r\n" +
            "^PW" + ((this.labelWidth / 100) * parseInt(selDPI)) + "\r\n" +
            "^LL" + ((this.labelHeight / 100) * parseInt(selDPI)) + "\r\n" +
            "^PON\r\n";
        var bufferData = "";
        ...
        for (var i = 0; i < this.currentLayer; i++) {
            if (this.elements[i]) {
                bufferData += this.elements[i].getZPLData();
                data += this.elements[i].toZPLPrint(this.labelX, this.labelY, this.labelHeight, this.labelWidth, this.hh, this.ww);
            }
        }
    ...
    this.saveDesign = function () {
        ...
        design = {};
        design.elements = [];
        design.elements.push({ lblHeight: $(".clsHeight").val(), lblWidth: $(".clsWidth").val(), DPI: $(".dpiCls").val(), Orientation: $(".orientationCls").val() });
        for (var i = 0; i < this.currentLayer; i++)
            if (this.elements[i])
                design.elements.push(this.elements[i].save());


        return JSON.stringify(design);
    }

    this.loadDesign = function (text) {

        var element, _class, design;
        ...
        try {
            ...
            $('.clsHeight').val(design.elements[0].lblHeight);
            $('.clsWidth').val(design.elements[0].lblWidth);
            var heigth = $('.clsHeight').val();
            var width = $('.clsWidth').val();
            if (heigth == "") {
                $('.clsHeight').val(4);
            }
            if (width == "") {
            ...
            $('.clsWidth').trigger('blur');
            $('.clsHeight').trigger('blur')

            $('.dpiCls').val(design.elements[0].DPI);
            $('.orientationCls').val(design.elements[0].Orientation);
            for (var i = 0; i < design.elements.length; i++) {
                element = design.elements[i];
                
                if (element.type == 'label_text')
                    _class = com.logicpartners.designerTools.text;
                if (element.type == 'Barcode')
                    _class = com.logicpartners.designerTools.barcode;
                if (element.type == 'EAN13')
                    _class = com.logicpartners.designerTools.EAN13BarCode;
                if (element.type == 'UPC')
                    _class = com.logicpartners.designerTools.UPC;
                if (element.type == 'rectangle')
                    _class = com.logicpartners.designerTools.rectangle;
                if (element.type == 'twoD')
                    _class = com.logicpartners.designerTools.twoD;
                if (element.type == 'image')
                    _class = com.logicpartners.designerTools.image;
				if (element.type == 'strokerectangle')
                    _class = com.logicpartners.designerTools.strokerectangle;
                if (_class != undefined)
                    this.elements[this.currentLayer++] = _class.load(element);
            }
        }
        catch (e) {
            alert("The input data is corrupt (or is not a stored design)");
            this.clearCanvas();
        }
